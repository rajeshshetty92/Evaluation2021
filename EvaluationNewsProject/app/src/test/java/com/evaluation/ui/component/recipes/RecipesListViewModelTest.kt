...
class RecipesListViewModelTest {
    // Subject under test
    private lateinit var recipesListViewModel: NewsListViewModel
    ...
    fun `get Recipes List`() {
        ...
        //2-Call
        recipesListViewModel = NewsListViewModel(dataRepository)
        recipesListViewModel.getRecipes()
        //active observer for livedata
        recipesListViewModel.recipesLiveData.observeForever { }

        //3-verify
        val isEmptyList = recipesListViewModel.recipesLiveData.value?.data?.articles.isNullOrEmpty()
    ...
    fun `get Recipes Error`() {
        // Let's do an answer for the liveData
        val error: Resource<News> = Resource.DataError(NETWORK_ERROR)
    ...
    fun `search Success`() {
        // Let's do an answer for the liveData
        val recipe = testModelsGenerator.generateArticleModel()
